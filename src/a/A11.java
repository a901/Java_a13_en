...
public class A11 implements Serializable { //遊戲主控制區，背景生成/每日事件觸發，調用A11的function來渲染視窗
	...
	int population[],populationtribe[],yn,starvedie=0,violencedie=0,die=0;
	
	double populationgrow[],populationdy[],populationstarve[],populationviolence[];
	int saveprepare=0;
	...
	A11()
	{
		...
		for(int z=0;z<gg;z++)
		{
			...
			mainset(cx,cy,31+(int)(Math.random()*11171)%3,(int)(Math.random()*1121117)%41+8);
		}
		
		gg=(4900/300)*(4900/300);
		...
		for(int z=0;z<gg;z++)//  沙漠
		{
			...
			gh=(int)(Math.random()*1121117)%13+2;
			mainset(cx,cy,31+(int)(Math.random()*11171)%3,(int)(Math.random()*1121117)%73+17);
		}
		...
		for(int z=0;z<gg;z++)
		{
			...
			mountainset(cx,cy,2+(int)(Math.random()*1121117)%4,(int)(Math.random()*1121117)%29+7);
		}
		
		gg=(4900/400)*(4900/400);
		...
		for(int z=0;z<gg;z++)
		{
			...
			mountainset(cx,cy,2+(int)(Math.random()*1121117)%4,(int)(Math.random()*1121117)%71+21);
		}
		...
		for(int z=0;z<5000;z++)
		{
			...
			for(int x=0;x<bordery1;x++)
				mapall[z][x]=41;
			
			for(int x=bordery2;x<5000;x++)
				mapall[x][z]=41;			
	...
	void st() {
		...
		{	
			for(int i=0;i<8;i++)
			{
				newone( 1, 1,boy[(z%2)*10+i+1] , 2513+i*5,2505+z*10,(z%2)*10+i+1);
				newone( 2, 1,girl[(z%2)*10+i+1], 2515+i*5+3,2505+z*10,(z%2)*10+i+1);
			}
		}
		//pl[0]=new Place();
		
		//pl[0].create0(2500, 2500, 0);
	}
			 ...
			 public void run(){	  try {
				...
				//	System.out.print("day "+day+"\n");
					...
					if(a13.kk.stop==0) {
						...
						if(day>=30)
						{
							System.out.print("new year="+year+"\n");
							day=1;
							year+=1;
							if(year%10==1)
							{
	...
	void newdecade()
	{
		...
		populationviolence[yn]=(double)violencedie*100/(double)die;
		populationdy[yn]=die;
	...
	//	System.out.print("newone \n");
		...
		for(int z=1;z<bn;z++)
		{
			if(a13.a1.allbn>150000&&z>30000)
				return;
			if(a13.a1.allbn>1250000&&z>15000)
				return;
			if(a13.a1.allbn>6250000&&z>5000)
				return;
			if(b[z].s<0)
			{
				k=z;
		...
		if(w>=115&&(int)(Math.random()*17771)%4==1&&(((int)(Math.random()*12121)%((w-100)*(w-100)+10)<100)))
		{
			w+=3;
		}
		else if(w<=85&&(int)(Math.random()*127721)%3==1&&(((int)(Math.random()*12121)%((w-100)*(w-100)+10))<100))
		{
			w-=3;
		}
		...
		if(s>=115&&(int)(Math.random()*12121)%4==1&&((int)(Math.random()*12121)%((s-100)*(s-100)+10))<100)
		{
			s+=3;
		}
		else if(s<=85&&(int)(Math.random()*12121)%3==1&&((int)(Math.random()*12121)%((s-100)*(s-100)+10))<100)
		{
			s-=3;
		}
		...
		g=fak;
		if(b[fak].group>=0&&group[b[fak].group].leader>=0&&b[mak].group==b[fak].group)
		{
			b[k].group=b[fak].group;
			
			allb[b[k].k0].group1=b[fak].group;
		}
		else if(b[mak].group>=0&&group[b[mak].group].leader>=0)
		{
	...
	Boolean check(int h,int k,int fa,int ma)
	{
		...
		for(int z=0;z<allb[fa].kidn;z++)
		{
			...
			/*
			System.out.print("\n z "+z+"\n");
			System.out.print(" kid[z] "+allb[fa].kid[z]+"\n");
			System.out.print(" kidn "+allb[fa].kidn+"\n");
			System.out.print(" name "+allb[fa].name()+"\n");
			System.out.print(" name "+allb[fa].age()+"\n");
			System.out.print(" allbn "+allbn+"\n");
			*/
	...
	void newday()
	{
		...
		if((year<50&&dayr%227==1)||(year<150&&dayr%571==1)||(dayr%2311==1))
		{
		...
		if(dayr%39==1)
		{
			for(int x=borderx1-50;x<borderx2+50;x++)
			{
				for(int z=bordery1-50;z<bordery2-50;z++)
				{
					if(map[x][z]==151||map[x][z]==154)
					{
		...
		if(dayr%59==1)
		{
			for(int x=borderx1-50;x<borderx2+50;x++)
			{
				for(int z=bordery1-50;z<bordery2-50;z++)
				{
					...
					if((map[x][z]>=151&&map[x][z]<=154))
					{
						if(a13.a1.allb[map2[x][z]-1000].age()+a13.a1.allb[map2[x][z]-1000].borntime<year-3) {
							map[x][z]=-1;
							map2[x][z]=-1;
						}
					}
		...
		if(dayr%47==1||daydelay>=100)
		{
			...
			for(int x=1;x<498;x++)
			{
				if(x*10<bordery1||x*10+10>bordery2)
					continue;
				for(int z=1;z<498;z++)
				{g=0;
					for(int i=0;i<10;i++)
					{
						...
						for(int j=0;j<10;j++)
						{
							...
							if(z*10+j<bordery1||z*10+j>bordery2)
								continue;
		...
		for(int z=0;z<groupn;z++)
		{
			...
			if(group[z].leader<0&&group[z].dietime>=year-500) {
				
				if((int)(Math.random()*55521)%7==1||group[z].dietime>=year-1)
				a13.a1.group[z].changeleader();
